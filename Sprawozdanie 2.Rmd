...
source("rmix.R")
```
Funkcja "rmix" tworzy **n**-elementowy rozkład będący mieszanką dwóch innych rozkładów (**family_1**, **family_2**) o parametrach (**par_1**, **par_2**). **p** jest prawdopodobieństwem wystąpienia **pierwszego** typu rozkładu.
```{r }
...
params <- expand.grid(len = len, prob = prob)
x <- 0
tib <- tibble(sw_test = 0, ks_test = 0, jb_test = 0, prob = 0, len = 0)
for(y in 1:length(params[,1]))
{
  ...
  t <- ctp(ll)
  tib <- bind_rows(tib, bind_cols(t, prob = prob1, len = len1))
  x <- x+1
}
tib <- tib[-1,]
pp <- bind_rows(tib[,5], tib[,5], tib[,5])

melted_tib <- melt(tib[,-5], id = "prob")
melted_tib <- bind_cols(melted_tib, pp)
colnames(melted_tib) <- c("Propability", "Test", "Power", "Length")
